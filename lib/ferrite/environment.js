...
Environment.prototype.resolve = function resolve(addr) {
  if(!this.heap[ addr ]) throw new Error('Trying to resolve a pointer to an empty heap entry.');
  return this.heap[ addr ];
};

Environment.prototype.gc = function env_gc(marked) {
  marked.push(0 /* Nil */,
    this.proto['object'],
    this.proto['boolean'],
    this.proto['string'],
...
Environment.prototype.sweep = function sweep(marked) {
  this.debug('Env.sweep'/*, marked*/);
  this.gc(marked); // mark primal prototypes
