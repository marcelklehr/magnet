...
TypeFactory.proto.makeString = function(env) {
  ...
  // split
  object.properties['split'] = env.set( TypeFactory.makeNativeFunction(env, function(precScope, seperator) {
    ...
    var list = string.split(seperator).map(function(piece) {
      return env.set(TypeFactory.makeString(env, piece));
    });
    return env.set(TypeFactory.makeList(env, list));
  }));
