...
Scope.prototype.setPropertyPointer = function setPointer(identifier, addr) {
  if(identifier == 'nil') throw new Error('Can\'t assign to nil keyword');
  if(this.parent && !this.properties[identifier]) return this.parent.setPropertyPointer(identifier, addr);
...
Scope.prototype.execute = function execute( node ) {
  ...
  switch( node.type )
  {
    ...
    case NODE_BOOL:
      return this.env.set(new PrimalBoolean(this.env, node.value));
    
    case NODE_INT:
      return this.env.set(new PrimalInteger(this.env, node.value));
    
    case NODE_FLOAT:
      return this.env.set(new PrimalFloat(this.env, node.value));
    
    case NODE_STR:
      return this.env.set(new PrimalString(this.env, node.value));
    ...
    case NODE_EXPR:
      return this.env.set( this.execute( node.value ) );
      
    case NODE_VAR:
      var variable = this.execute(node.value);  // console.log('# read variable: <object>.'+variable.property+' -> '+variable.object.getPropertyPointer(variable.property));
    ...
    case NODE_OP:
      ...
      /* Comparison Operators */
        case OP_EQU:
          ...
          return obj.callMethod('==', [this.execute( node.children[1] )]);
        
        case OP_NEQ:
          var obj = this.env.resolve(this.execute( node.children[0] ));
          return this.env.resolve( obj.callMethod( '==' , [this.execute( node.children[1] )]) ).callMethod('!');
      ...
      /* Arithmetic Operatos */
        case OP_ADD:
          var obj = this.execute( node.children[0] );
          return obj.callMethod('+', this.execute( node.children[1] ));
        case OP_SUB:
          var obj = this.execute( node.children[0] );
          return obj.callMethod('-', this.execute( node.children[1] ));
        ...
        case OP_NEG:
          var obj = this.execute( node.children[0] );
          return obj.callMethod('*', [this.env.set(TypeFactory.makeInteger(this.env, -1))]);
      }
      break;
    ...
    case NODE_FUNC:
      ...
      var func = new PrimalFunction(this.env, this, node.value, params);// env, inheritScope, code, params
    ...
    case NODE_CALL:
      var identifier = (node.value.type == NODE_VAR) ? this.execute(node.value.value) : '<Closure>';
      ...
      // invoke function
      var ret = this.env.cast('function', func).invoke(args);
      if(ret != 0) {
