...
TypeFactory.proto.makeBoolean = function(env) {
  ...
  object.properties['=='] = env.set(TypeFactory.makeNativeFunction(env, function(precScope, obj) {
    ...
    return env.set(TypeFactory.makeBoolean(env, equ));
  }));
  
  // ||
  object.properties['||'] = env.set(TypeFactory.makeNativeFunction(env, function(precScope, obj) {
    var or = env.cast('boolean', this).getVal() || env.cast('boolean', env.resolve(obj)).getVal();
    return env.set(TypeFactory.makeBoolean(env, or));
  }));
  ...
  object.properties['!'] = env.set(TypeFactory.makeNativeFunction(env, function(precScope) {
    ...
    return env.set(TypeFactory.makeBoolean(env, not));
  }));
  
  // read
  object.properties['read'] = env.set(TypeFactory.makeNativeFunction(env, function(precScope) {
    return env.set(TypeFactory.makeString(env, ( env.cast('boolean', this).getVal() ) ? 'true' : 'false'));
  }));
